...
const DocumentSchema = Yup.object().shape({
  name: Yup.string().required('Document name is required').max(255),
  type: Yup.string().required('Document type is required').max(50),
  folder_id: Yup.number().nullable(),
});

const AddDocumentForm: React.FC<AddDocumentFormProps> = ({ onSubmit, currentFolderId, folders }) => {
  return (
    <Formik
      initialValues={{ name: '', type: '', folder_id: currentFolderId }}
      ...
      {({ isSubmitting }) => (
        <Form>
          ...
          <FormGroup>
            ...
            <Field name="type" type="text" placeholder="Enter document type (e.g., PDF, DOCX)" />
